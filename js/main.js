    ...
    $.widget("q4.stories", {
                ...
                multi: (
                    '<div id="{{SeoName}}" class="story-item multi col{{cls}}">' +
                        ...
                        '<div class="col story-image fade-in">' +
                            '{{#LinkToDetailPage}}<a href="{{LinkToDetailPage}}"{{ext}}>{{/LinkToDetailPage}}' +
                                '<img data-src="{{ThumbnailPath}}" alt="{{Headline}}">' +
                        ...
                        '<div class="col story-content">' +
                            ...
                            '{{#LinkToDetailPage}}<a href="{{LinkToDetailPage}}"{{ext}} class="details arrow fade-in"></a>{{/LinkToDetailPage}}' +
                        '</div>' +
                    '</div>'
                ),
                'multi-alt': (
        ...
        _onHeroScroll: function() {
            ...
            $('.scroll-button').on('click', function(){
                inst._scrollTo( $('.content-container').offset().top );
            });
        },
        ...
        _afterContentLoaded: function(){
            ...
            var hashCheck = setTimeout(function(){
                if (location.hash.length){
                    inst._scrollTo( $( location.hash ).offset().top);
                }
                clearTimeout(hashCheck);
            }, 1000);
            ...
            $(window).on( 'DOMContentLoaded load resize scroll', function(){
                ...
                var animated = $viewport.filter('.animate');

                if ( animated ) {

                    if ( animated.find('.count-to').not('.complete').length ){
                        $viewport.filter('.animate').find('.count-to').addClass('complete');
                        inst._countTo( $viewport.filter('.animate').find('.count-to .timer') );
                    }
                    if (animated.find('.story-slider').not('.complete').length) {
                        var slick = animated.find('.story-slider').addClass('complete').slick(inst.options.slickOpts);
...
var q4 = {
    ...
    _onMenuClick: function() {
        $('.nav-toggle').on('click', function(){
            $('body').toggleClass('toggle-nav-open');
        });
    },
    _onMenuClose: function() {
        $('.nav-container').on('click', '.close', function(){
            $('body').toggleClass('toggle-nav-open');
        });
    },
...
;(function (window, $, undefined) {
    ...
    */
    ...
    $(function () {
        ...
        baseHref = '';
        fileTypes = /\.(zip|exe|dmg|pdf|doc.*|xls.*|ppt.*|mp3|txt|rar|html|wma|mov|avi|wmv|flv|wav)(\?.*)?$/i;
        domainRegex = /^https?:\/\/([^\/:?#]+)(?:[\/:?#]|$)/i;
        httpRegex = /^https?\:\/\//i;
